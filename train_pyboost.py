...
def custom_mean_rowwise_rmse( y_true, y_pred):
    """
    Custom metric to calculate the Mean Rowwise Root Mean Squared Error (RMSE) using NumPy.
    ...
    Returns:
    - Mean Rowwise RMSE as a scalar value.
    """
    mse_per_row = np.mean((y_true - y_pred) ** 2, axis=1)
...
def MT_cross_validate_model(de_train, id_map, test_preds_svr_with_emb_v1, oof_preds_svr_with_emb_v1, test_preds_svr_with_emb_v2, oof_preds_svr_with_emb_v2, agg_funcs, all_chem_emb, te_chem_emb):
    de_cell_type = de_train.iloc[:, [0] + list(range(5, de_train.shape[1]))]
    de_sm_name = de_train.iloc[:, [1] + list(range(5, de_train.shape[1]))]
    target_cols = de_train.columns.to_list()[5:]
    ...
    for fold, (train_ids, test_ids) in enumerate(folds_index_data_MT):
        ...
        va_svr = np.concatenate([va_svr_v1, va_svr_v2, va_chem_emb],axis=1)
        ...
        va_svr_tsvd = tsvd_svr.transform(va_svr)
        te_svr_tsvd = tsvd_svr.transform(te_svr)
                
        tr_data = X_train_encoded
        va_data = X_val_encoded
        te_data = X_test_encoded
        
        tr_data = np.concatenate([tr_data, tr_svr_tsvd],axis=1)
        va_data = np.concatenate([va_data, va_svr_tsvd],axis=1)
        te_data = np.concatenate([te_data, te_svr_tsvd],axis=1)
        
        for func in agg_funcs:
            
            tr_cell_type = agg_feas[func]['tr_cell_type'][train_ids]
            va_cell_type = agg_feas[func]['tr_cell_type'][test_ids]
            ...
            va_sm_name_tsvd = tsvd_sm_name.transform(va_sm_name)
            te_sm_name_tsvd = tsvd_sm_name.transform(te_sm_name)
            
    
        for func in agg_funcs:
            tr_data = np.concatenate([tr_data, tr_cell_type_tsvd],axis=1)
            va_data = np.concatenate([va_data, va_cell_type_tsvd],axis=1)
            te_data = np.concatenate([te_data, te_cell_type_tsvd],axis=1)

            tr_data = np.concatenate([tr_data, tr_sm_name_tsvd],axis=1)
            va_data = np.concatenate([va_data, va_sm_name_tsvd],axis=1)
        ...
        va_data_pred = model.predict(va_data)
        te_data_pred = model.predict(te_data)

        va_data_pred = tsvd.inverse_transform(va_data_pred)
        te_data_pred = tsvd.inverse_transform(te_data_pred)

        score = custom_mean_rowwise_rmse(de_train.loc[test_ids, target_cols].values, va_data_pred)
        mrrmse_scores.append(score)
    ...
    print('################')
    return average_predictions, model_list


def save_models(model_list, model_path):
    for i, model in enumerate(model_list):
        joblib.dump(model, os.path.join(model_path, f'model_{i + 1}.joblib'))
...
if __name__ == '__main__':
    ...
    test_preds, model_list = MT_cross_validate_model(df_de_train, id_map, test_preds_svr_with_emb_v1, oof_preds_svr_with_emb_v1, test_preds_svr_with_emb_v2, oof_preds_svr_with_emb_v2, agg_funcs, all_chem_emb, te_chem_emb)
    save_models(model_list, PYBOOST_MODEL_DIR)
    save_submission(sample_submission, test_preds, SUBMISSION_DIR)
